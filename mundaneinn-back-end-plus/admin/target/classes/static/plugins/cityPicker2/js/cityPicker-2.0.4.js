...
(function ($, window) {
    ...
    var defaults = {
        ...
        streetUrl: '/static/plugins/cityPicker2/town/{json}.json',
        level: 3,
    ...
    var effect = {
        ...
        seTemplet: function () {
            var config = this.options,
                selectemplet = '',
                placeholder, field, value, forbid, citygrade, active, hide,
            ...
            for (var i = 0; i < config.level; i++) { //循环定义的级别
                ...
                value = config.selectpattern[i].value; //默认值
                ...
                if (config.renderMode) {
                    ...
                    //原生
                    selectemplet += '<select id="' + citygrade + '" name="' + field + '" data-index="' + i + '" class="' + citygrade + '">'
