...
public class SysLogAspect {
     ...
     * 配置织入点 @sysLog注解
     */
    @Pointcut("@annotation(com.api.config.sysLog.SysLog)")
     ...
     */
/*    @Pointcut("execution(* com.api.sys.controller..*.*(..))")
    ...
    private void saveSysLog(ProceedingJoinPoint joinPoint, long time, final Exception e, Object jsonResult) {
        ...
        //操作人员账号
        sysLog.setOperUserAccount(clientUserEntity.getAccount());
        sysLog.setOperUserType("3");
        sysLog.setOperUserId(clientUserEntity.getId());
        if (e != null) {
            sysLog.setStatus(1);
